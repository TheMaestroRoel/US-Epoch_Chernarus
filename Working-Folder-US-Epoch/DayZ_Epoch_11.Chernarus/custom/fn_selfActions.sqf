	...
	*/
	...
	if ((dayz_myCursorTarget != _cursorTarget) and _isVehicle and !_isMan and _hasToolbox and (damage _cursorTarget < 1) and !_isDisallowRepair) then {
		if (s_player_repair_crtl < 0) then {
			...
			_menu1 = dayz_myCursorTarget addAction [localize "STR_EPOCH_PLAYER_SALVAGEV", "custom\SalvageVehicle\salvage_vehicle.sqf",_cursorTarget, 0, true, false, "",""];
...
if (s_player_deploybike6 < 0) then {
                s_player_deploybike6 = player addaction[("<t color=""#007ab7"">" + ("Re-Pack Mozzie") +"</t>"),"spawnbike\bike6.sqf","",5,false,true,"", ""];
        };
} else {
...
if (s_player_deploybike5 < 0) then {
                s_player_deploybike5 = player addaction[("<t color=""#007ab7"">" + ("Upgrade to Mozzie") +"</t>"),"spawnbike\bike5.sqf","",5,false,true,"", ""];
        };
} else {
